...
fn fixtures_core_decode() {
    ...
    let manifest: serde_json::Value =
        serde_json::from_str(&manifest_bytes).expect("parse manifest");
            ...
            if let Some(json_path) = expect.get("json").and_then(|v| v.as_str()) {
                if !rust_skip {
                    ...
                    let expected_json: serde_json::Value =
                        serde_json::from_str(&expected_bytes).expect("parse expected json");

                    let actual_json_str = to_json(&value).expect("to_json");
                    let actual_json: serde_json::Value =
                        serde_json::from_str(&actual_json_str).expect("parse actual json");
