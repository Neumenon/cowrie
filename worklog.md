# SJSON Final Sprint Plan (Atomic, Testable Tickets)
...
Tickets:
- S1-1 Create core fixtures in `testdata/fixtures/core/` (JSON + SJSON bytes + manifest entries).  
  Validation: `python tools/fixtures/validate_manifest.py`
...
- S1-3 Rust fixture runner for core cases in `rust/tests/fixtures_core.rs`.  
  Validation: `cargo test -p sjson fixtures_core`
...
- S2-6 Rust: ensure encode/decode covers ML + graph tags (if gaps exist).  
  Validation: `cargo test -p sjson fixtures_ml_graph`
...
- S3-3 Rust: add `DecodeOptions` with defaults matching Go; enforce per‑type limits.  
  Validation: `cargo test -p sjson fixtures_limits`
...
- S4-4 Rust: implement `skip_hints` + tests.  
  Validation: `cargo test -p sjson hints`
...
- S6-3 Rust: size‑limited decompress; verify `len == orig_len`; map errors.  
  Validation: `cargo test -p sjson framed`
...
- S6-6 C: implement `sjson_decode_framed` with size limit + tests.  
...
Tickets:
- S7-1 Implement `sjson_encode_framed` with gzip/zstd optional builds + raw fallback.  
